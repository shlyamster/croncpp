//
//  Copyright © 2018 Marius Bancila. All rights reserved.
//
...
enum class cron_field { second, minute, hour_of_day, day_of_week, day_of_month, month, year };
...
struct bad_cronexpr : public std::runtime_error {
 public:
    explicit bad_cronexpr(std::string_view message) : std::runtime_error(message.data()) {}
};
...
struct cron_standard_traits {
    ...
    static const inline std::vector<std::string> MONTHS = {"NIL", "JAN", "FEB", "MAR", "APR", "MAY", "JUN",
...
struct cron_oracle_traits {
    ...
    static const uint8_t CRON_MIN_DAYS_OF_WEEK = 1;
    static const uint8_t CRON_MAX_DAYS_OF_WEEK = 7;

    static const uint8_t CRON_MIN_DAYS_OF_MONTH = 1;
    static const uint8_t CRON_MAX_DAYS_OF_MONTH = 31;

    static const uint8_t CRON_MIN_MONTHS = 0;
    static const uint8_t CRON_MAX_MONTHS = 11;

    static const uint8_t CRON_MAX_YEARS_DIFF = 4;

    static const inline std::vector<std::string> DAYS = {"NIL", "SUN", "MON", "TUE", "WED", "THU", "FRI", "SAT"};
    static const inline std::vector<std::string> MONTHS = {"JAN", "FEB", "MAR", "APR", "MAY", "JUN",
                                                           "JUL", "AUG", "SEP", "OCT", "NOV", "DEC"};
};
...
inline bool operator==(cronexpr const& e1, cronexpr const& e2) {
    return e1.seconds == e2.seconds && e1.minutes == e2.minutes && e1.hours == e2.hours &&
           e1.days_of_week == e2.days_of_week && e1.days_of_month == e2.days_of_month && e1.months == e2.months;
}

inline bool operator!=(cronexpr const& e1, cronexpr const& e2) { return !(e1 == e2); }

inline std::string to_string(cronexpr const& cex) {
    return cex.seconds.to_string() + " " + cex.minutes.to_string() + " " + cex.hours.to_string() + " " +
           cex.days_of_month.to_string() + " " + cex.months.to_string() + " " + cex.days_of_week.to_string();
}
...
inline std::string to_upper(std::string text) {
    std::transform(std::begin(text), std::end(text), std::begin(text), static_cast<int (*)(int)>(std::toupper));
...
inline uint8_t to_uint8_t(std::string_view text) {
    try {
...
static std::string replace_ordinals(std::string text, std::vector<std::string> const& replacement) {
    for (size_t i = 0; i < replacement.size(); ++i) {
        ...
        if (std::string::npos != pos) text.replace(pos, 3, std::to_string(i));
    }

    return text;
}

static std::pair<uint8_t, uint8_t> make_range(std::string_view field, uint8_t const minval, uint8_t const maxval) {
    uint8_t first = 0;
    uint8_t last = 0;
    if (field.size() == 1 && field[0] == '*') {
        ...
        first = to_uint8_t(field);
        last = first;
    } else {
        auto parts = utils::split(field, '-');
        if (parts.size() != 2) throw bad_cronexpr("Specified range requires two fields");

        first = to_uint8_t(parts[0]);
        last = to_uint8_t(parts[1]);
    }
...
template <size_t N>
static void set_cron_field(std::string_view value, std::bitset<N>& target, uint8_t const minval, uint8_t const maxval) {
    if (value.length() > 0 && value[value.length() - 1] == ',') throw bad_cronexpr("Value cannot end with comma");

    auto fields = utils::split(value, ',');
    if (fields.empty()) throw bad_cronexpr("Expression parsing error");
    ...
    for (auto const& field : fields) {
        if (!utils::contains(field, '/')) {
            ...
            auto parts = utils::split(field, '/');
            if (parts.size() != 2) throw bad_cronexpr("Incrementer must have two fields");
            ...
            auto delta = detail::to_uint8_t(parts[1]);
            if (delta <= 0) throw bad_cronexpr("Incrementer must be a positive value");
...
inline void add_to_field(std::tm& date, cron_field const field, int const val) {
    ...
    if (INVALID_TIME == utils::tm_to_time(date)) throw bad_cronexpr("Invalid time expression");
}
...
inline void reset_all_fields(std::tm& date, std::bitset<7> const& marked_fields) {
    for (size_t i = 0; i < marked_fields.size(); ++i) {
        if (marked_fields.test(i)) reset_field(date, static_cast<cron_field>(i));
    }
}
...
template <size_t N>
static size_t find_next(std::bitset<N> const& target, std::tm& date, unsigned int const minimum,
                        unsigned int const maximum, unsigned int const value, cron_field const field,
...
template <typename Traits>
static size_t find_next_day(std::tm& date, std::bitset<31> const& days_of_month, size_t day_of_month,
                            std::bitset<7> const& days_of_week, size_t day_of_week,
...
static bool find_next(cronexpr const& cex, std::tm& date, size_t const dot) {
    ...
    auto updated_second = find_next(cex.seconds, date, Traits::CRON_MIN_SECONDS, Traits::CRON_MAX_SECONDS, second,
                                    cron_field::second, cron_field::minute, empty_list);
    ...
    auto update_minute = find_next(cex.minutes, date, Traits::CRON_MIN_MINUTES, Traits::CRON_MAX_MINUTES, minute,
                                   cron_field::minute, cron_field::hour_of_day, marked_fields);
    ...
    auto updated_hour = find_next(cex.hours, date, Traits::CRON_MIN_HOURS, Traits::CRON_MAX_HOURS, hour,
    ...
    auto updated_day_of_month =
        find_next_day<Traits>(date, cex.days_of_month, day_of_month, cex.days_of_week, day_of_week, marked_fields);
    ...
    auto updated_month = find_next(cex.months, date, Traits::CRON_MIN_MONTHS, Traits::CRON_MAX_MONTHS, month,
                                   cron_field::month, cron_field::year, marked_fields);
    if (month != updated_month) {
        if (date.tm_year - dot > Traits::CRON_MAX_YEARS_DIFF) return false;
...
static cronexpr make_cron(std::string_view expr) {
    ...
    if (expr.empty()) throw bad_cronexpr("Invalid empty cron expression");

    auto fields = utils::split(expr, ' ');
    fields.erase(std::remove_if(std::begin(fields), std::end(fields), [](std::string_view s) { return s.empty(); }),
                 std::end(fields));
    if (fields.size() != 6) throw bad_cronexpr("cron expression must have six fields");

    detail::set_cron_field(fields[0], detail::cron_field_ref<detail::cron_field::second>(cex), Traits::CRON_MIN_SECONDS,
                           Traits::CRON_MAX_SECONDS);
    detail::set_cron_field(fields[1], detail::cron_field_ref<detail::cron_field::minute>(cex), Traits::CRON_MIN_MINUTES,
                           Traits::CRON_MAX_MINUTES);
    detail::set_cron_field(fields[2], detail::cron_field_ref<detail::cron_field::hour_of_day>(cex),
                           Traits::CRON_MIN_HOURS, Traits::CRON_MAX_HOURS);
...
static std::tm cron_next(cronexpr const& cex, std::tm date) {
    ...
    if (!detail::find_next<Traits>(cex, date, date.tm_year)) return {};
...
static std::time_t cron_next(cronexpr const& cex, std::time_t const& date) {
    ...
    if (!detail::find_next<Traits>(cex, *dt, dt->tm_year)) return INVALID_TIME;
